...
const loginUser = async(payload:TLoginUser) =>{
    ...
    const accesstoken = jwt.sign(tokenUser,env.jwt_access_secret as string,{expiresIn:'10d'});
...
const changePasswordToDB = async(userData:JwtPayload,payload:{oldPassword:string,newPassword:string})=> {
    // check if user exist using static method
    // const user = await UserModel.isUserExistByCustomId(payload.id).select('+password');
    const user = await UserModel.findOne({id:userData.userId}).select('+password'); // tell with "+" sign to select password since in schema we have used {select:0}
    ...
    // check if the password match
    const isPasswordMatched = await UserModel.isPasswordMatched(payload.oldPassword,user.password);
    if (!isPasswordMatched) {
        throw new AppError(httpStatus.FORBIDDEN, `Invalid user id or password!`);
    }

    // hash new password
    const newHashedPassword = await bcrypt.hash(payload.newPassword,Number(env.BCRYPT_SALT_ROUNDS));
